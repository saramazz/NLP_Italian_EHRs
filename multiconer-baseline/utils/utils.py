...
def create_iob_entities(entities):
    d = {}
    for e in entities:
        d['B-{}'.format(e)] = len(d)
        d['I-{}'.format(e)] = len(d)
...
ris_iob = create_iob_entities(('ACTI', 'DISO', 'DRUG', 'SIGN', 'BODY', 'TREA'))
proximity_care_iob = create_iob_entities(('BBDx', 'BBSx', 'CODP', 'DY', 'EF', 'FA', 'HY', 'NODB', 'NODY', 'NYHA', 'RS'))
...
def parse_args():
    ...
    p.add_argument('--standard_output', action='store_true', help='print output on stdout')
...
def get_tagset(tagging_scheme):
    if 'conll' in tagging_scheme:
        ...
        return weibo_iob
    elif 'ris' in tagging_scheme:
        return ris_iob
    elif 'proximity_care' in tagging_scheme:
        return proximity_care_iob
